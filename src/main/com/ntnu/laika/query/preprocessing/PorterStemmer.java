 ...
 * under the License.
 *
 * The Original Code is PorterStemmer.java.
 *
 ...
 * @author Gianni Amati, (Java) optimised by Craig Macdonald
...
public class PorterStemmer {
	 ...
	 */
	public final String processTerm(String t)
	{
		return (t == null) ? null : stem(t);
	}
	/* end of termpipeline implementation */
	/** 
	 * cons(i) is TRUE <=> b[i] is a consonant. 
	 */
	protected static boolean cons(int i) {
		switch (b[i]) {
			case 'a' :
			case 'e' :
			case 'i' :
	...
	protected static boolean consonantinstem() {
		int i;
		for (i = k0; i <= j; i++)
			if (cons(i))
				return true;
	 ...
	 */
	protected static boolean cvc(int i) {
	...
	protected static void defineBuffer(String s) {
	 ...
	 */
	protected static boolean doublec(int j) {
		if (j < k0 + 1)
			return false;
		if (b[j] != b[j - 1])
			return false;
	 ...
	 */
	protected static boolean ends(String s) 
	{  
		final int length = s.length();
	 ...
	 * k and j. 
	 */
	protected static void setto(int i1, int i2, String str) {
	 ...
	 */
	protected static boolean vowelinstem() {
		int i;
		for (i = k0; i <= j; i++)
			if (!cons(i))
				return true;
