 ...
 * @version $Id $.
 */
public class AND_DAAT_LP_MaxScore_QueryProcessing extends QueryProcessing{
	
	public AND_DAAT_LP_MaxScore_QueryProcessing(DiskInvertedIndex index, Statistics stats){
		super(index, stats);
	}
...
loop:  while (true) {
			...
			for (i = 1; i < numTerms && score + accScores[i] >= reqScore; i++){
				if (iterators[i].skipTo(doc)){
					...
					} else if (iterators[0].skipTo(_doc)){
						continue loop;
					} else break loop;
				} else break loop;
			}			
