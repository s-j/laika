...
public void boot(){
	...
	if (isRootNode){
		...
		if (processortype == LESTER || processortype == SKIPLESTER) lester_L = Integer.parseInt(subargs[5]);

		System.out.println("method:" + processortypeToName() + " skips:" + Constants.USE_SKIPS + " K:" + Constants.MAX_NUMBER_OF_RESULTS + " L?:" + lester_L + " testlog" + (subargs.length > 6 ? subargs[6] : ""));
		
		//QueryDispatcher2 dispatcher = new QueryDispatcher2(this, Integer.parseInt(subargs[0]), Integer.parseInt(subargs[1]), 
		//		Integer.parseInt(subargs[2]), processortype == MSDOR);
		TPQueryDispatcher dispatcher = new TPQueryDispatcher(this, Integer.parseInt(subargs[0]), Integer.parseInt(subargs[1]), 
				Integer.parseInt(subargs[2]), processortype == MSDOR, subargs.length > 6 ? subargs[6] : "");
		...
		TPWorkerQueryProcessing worker = new TPWorkerQueryProcessing(this);
...
private String processortypeToName() {
	switch (processortype) {
		...
		case (LESTER) : 	return "Lester";
		case (SKIPLESTER) : return "SkipLester";
		case (AND) : 		return "And";
		default : 			return "Or";
	}
}
...
private class ShutdownRequestHandler implements MessageHandler{
	private TPWorkerQueryProcessing worker;
	
	public ShutdownRequestHandler(TPWorkerQueryProcessing worker){
		this.worker = worker;
	}
...
public static void main(String[] args){
	TPKernel kernel = new TPKernel(args);
	kernel.boot();
}
