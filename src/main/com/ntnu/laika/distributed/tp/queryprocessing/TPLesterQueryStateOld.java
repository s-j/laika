 ...
 * @version $Id $.
 */
public class TPLesterQueryStateOld implements TPQueryState{
	...
	public TPLesterQueryStateOld(TPQueryBundle bundle, Statistics stats, DiskInvertedIndex inv, boolean skipOpt, int lester_L){
	...
	public TPResultState processQuery(){
		...
		for (int i = 0; i < numTerms; i++) {
			...
			if (lester_L > numPointers){
				...
				//store current max as it will be stored anyway
				while (inacc.hasMore() && inacc.getDocId() < maxdoc ){
					if (inacc.getScore() >= lester_vt)
						outacc.addLast(inacc.getDocId(), inacc.getScore());
					inacc.next();
				}
			
				score = iter.getFakeScore(lester_ht, FastDocLength.getNumberOfTokens(maxdoc));
				if (inacc.hasMore() && inacc.getDocId() == maxdoc)
					score += inacc.getScore();
				if (score >= lester_vt)
					outacc.addLast(maxdoc, score);
			
				remPointers -= lester_p; 
				lester_ht--;
			
				//System.out.println("First: " + lester_p + " " + lester_ht + " " + lester_vt + " " + lester_L);
			} else  {
				...
				/*
				int lester_ht1 = 1;
				while (iter.getFakeScore(lester_ht1, avgDL) < lester_vt && lester_ht1 < maxtf){
					lester_ht1++;
				} 
				System.out.println(">>" + lester_ht + " " + lester_ht1);
				if (lester_ht == maxtf){
					lester_p = numPointers+1;
					andmode = skipOpt;
				}*/
