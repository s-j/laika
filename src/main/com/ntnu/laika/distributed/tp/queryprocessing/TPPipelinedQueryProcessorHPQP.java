...
import com.ntnu.laika.distributed.tp.TPKernelHPQP;
 ...
 * @version $Id $.
 */
public class TPPipelinedQueryProcessorHPQP implements TPQueryProcessor{
...
//	private ExecutorService workpool;
	...
	public TPPipelinedQueryProcessorHPQP(LocalIndex index, NetworkServer server, ExecutorService workpool) {
//		this.workpool = workpool;
	...
	public void processQuery(TPQueryBundle bundle) {
		...
		switch (TPKernelHPQP.processortype){
			case (TPKernelHPQP.MSDOR): state = new TPMSDQueryState(bundle, globalStats, inv); break;	
			case (TPKernelHPQP.AND) : state = new TPANDQueryState(bundle, globalStats, inv); break;
		...
		} else if ((TPKernelHPQP.processortype == TPKernelHPQP.AND) && res.accset.size() == 0){		//since we do AND processing: no accs -> terminate!
